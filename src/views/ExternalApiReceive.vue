  ...
  <div class="perm-page">
    <div class="page-head">
      <div>
        ...
        <p class="page-desc">
          接收外部应用推送数据，按预定规则解析并校验格式与内容，提取有效信息。
        </p>
      </div>
    ...
    <section class="config-card">
      ...
      <el-form label-width="88px" size="small">
        <el-form-item label="API Key">
          ...
          >
            <el-option
              v-for="k in apiKeys.filter((x) => x.enabled)"
              :key="k.id"
              :label="`${k.name} (${k.apiKey.slice(0, 16)}...)`"
              :value="k.apiKey"
            />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="请求体 JSON">
          <el-input
            ...
            type="textarea"
            :rows="12"
            placeholder="粘贴外部系统推送的 JSON 数据"
          />
        </el-form-item>
        <el-form-item>
          <el-button type="primary" :loading="processing" @click="handleReceive"
            >接收并解析</el-button
          >
    ...
    <section v-if="parseResult" class="config-card">
      ...
      <div
        v-if="parseResult.warnings && parseResult.warnings.length"
        class="warn-list"
      >
        <div v-for="(w, i) in parseResult.warnings" :key="i">{{ w }}</div>
      </div>
      <p v-if="parseResult.recordCount != null" class="result-meta">
        共 {{ parseResult.recordCount }} 条记录
      </p>
    </section>
  ...
  methods: {
    ...
    loadSample() {
      this.payloadText = JSON.stringify(
        { ...SAMPLE_PAYLOAD, sourceType: this.sourceType },
        null,
        2,
      );
    },
    ...
    handleReceive() {
      ...
      try {
        const result = processExternalApiRequest(
          this.apiKey,
          this.getPayload(),
        );
        this.parseResult = result.parsed || {
