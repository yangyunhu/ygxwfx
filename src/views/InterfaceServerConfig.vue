<template>
  <div class="perm-page">
    ...
    <section class="config-card">
      ...
      <p class="section-tip">
        配置 API 接口所在服务器的 IP 地址或域名，以及接口监听的端口号。
      </p>
      <el-form
        ref="serverFormRef"
        :model="serverForm"
      ...
      >
        ...
        <el-form-item
          :label="serverForm.hostType === 'ip' ? 'IP 地址' : '域名'"
          prop="serverHost"
        >
          <el-input
            v-model="serverForm.serverHost"
            style="width: 360px"
            placeholder="如 api.ygxwfx.csg.cn 或 10.12.8.100"
          />
        </el-form-item>
        ...
        <el-form-item label="API 基础路径" prop="apiBasePath">
          <el-input
            v-model="serverForm.apiBasePath"
            style="width: 360px"
            placeholder="/api/v1/external"
          />
        </el-form-item>
        <el-form-item label="连接超时">
          <el-input-number v-model="serverForm.timeout" :min="5" :max="120" />
          秒
        </el-form-item>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      serverRules: {
        ...
        apiBasePath: [
          { required: true, message: "请填写基础路径", trigger: "blur" },
        ],
      },
    };
  },
  ...
  methods: {
    ...
    handleReset() {
      this.$confirm("确定恢复默认服务器端口配置？", "恢复默认", {
        type: "warning",
      })
        .then(() => {
          resetServerConfig();
