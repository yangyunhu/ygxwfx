  ...
  <div class="perm-page">
    <div class="page-head">
      <div>
        ...
        <p class="page-desc">
          通过 API Key 认证机制验证调用方身份与权限，确保接口安全访问。
        </p>
      </div>
      <el-button
        type="primary"
    ...
    >
      ...
      >
        ...
        <el-form-item label="应用ID"
          ><el-input v-model="form.appId" placeholder="自动生成"
        /></el-form-item>
...
export default {
  ...
  computed: {
    ...
    formTitle() {
      return this.formMode === "create" ? "新增 API Key" : "编辑 API Key";
    },
  },
  mounted() {
    this.reload();
  ...
  methods: {
    ...
    permLabel(p) {
      return p === "receive" ? "接收" : p === "transform" ? "转换" : p;
    },
    openCreate() {
    ...
    resetForm() {
      this.$refs.keyForm && this.$refs.keyForm.resetFields();
    },
    submitForm() {
    ...
    handleDelete(row) {
      this.$confirm(`确定删除「${row.name}」的 API Key？`, "删除", {
        type: "warning",
      })
        .then(() => {
          deleteApiKey(row.id);
